...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n + 1):
        start1 = rg.Point(point.x, point.y + 30 * k)
        endpoint1 = rg.Point(point.x + length, point.y + 30 * k)
        line1 = rg.Line(start1, endpoint1)
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        endpoint = rg.Point(point.x + 100, point.y - (100 - ((200 / (n - 1) * k))))
        line = rg.Line(point, endpoint)
        line.attach_to(window)
        window.render(0.25)
